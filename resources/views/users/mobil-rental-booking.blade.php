...
<main class="flex-grow relative w-full">
            ...
            <form id="booking-form" action="#" method="POST" enctype="multipart/form-data" onsubmit="return false;">
                ...
                <div id="antar-form" class="delivery-form-content {{ $defaultMethod == 'antar' ? '' : 'hidden' }}">
                    ...
                    <div class="mb-6">
                        ...
                        {{-- $defaultMethod di sini adalah metode PENGIRIMAN (antar/jemput),
                             ditetapkan ulang di blok @php di atas. --}}
                        ...
                        <div id="transfer-payment-mobil" class="payment-content">
                        ...
                        <!-- Cash Payment Card -->
                        <div id="cash-payment" class="payment-content {{ $hasTransfer && $defaultMethod === 'antar' ? 'hidden' : '' }}">
    ...
    <div id="confirmation-modal" class="fixed inset-0 bg-black/50 hidden items-center justify-center z-50" style="display: none;">
        <div class="bg-white rounded-3xl p-8 max-w-md w-full mx-4 shadow-2xl transform transition-all">
            <div class="text-center">
                <img src="{{ asset('Admin/img/illustrations/isewalogo.webp') }}" alt="SiladesBeng Logo" class="w-40 mx-auto mb-6">
    ...
    {{-- Pemilih metode bayar sewa mobil. Dipisah dari skrip pengiriman form
         agar tidak mengganggu alur FormData yang sudah ada. --}}
    <script>
        window.pilihMetodeMobil = function (metode) {
            var input = document.getElementById('payment-method-hidden');
            if (input) {
                input.value = metode;
                input.dataset.pilihanAntar = metode;
            }
            ...
            [['btn-mobil-transfer', 'transfer'], ['btn-mobil-tunai', 'tunai']].forEach(function (pasangan) {
        ...
        // Dipanggil saat warga berpindah antara Antar dan Jemput. Jemput = ambil
        // sendiri, bayarnya di tempat; panel transfer tidak ikut ditampilkan di
        // sana sehingga metodenya dikunci ke tunai.
        window.kunciMetodeBayarMobil = function (pengiriman) {
            var input = document.getElementById('payment-method-hidden');
            if (!input) return;
        ...
        // dari Jemput lalu pindah ke Antar mendapat panel yang cocok dengan
        // tombol yang tersorot.
        (function () {
            var input = document.getElementById('payment-method-hidden');
...
function updateTotalsJemput() {
        ...
        deliveryCards.forEach(card => {
            card.addEventListener('click', function() {
                ...
                // JEMPUT, metode bayar harus kembali ke tunai - kalau tidak, form
                // mengirim payment_method=transfer tanpa bukti apa pun dan ditolak
                // server tanpa ada kolom unggah yang terlihat.
                if (typeof window.kunciMetodeBayarMobil === 'function') {
                    window.kunciMetodeBayarMobil(method);
                }
        ...
        const distanceKm = getEl('distance_km');
        if (distanceKm) distanceKm.addEventListener('input', updateTotals);
        ...
        const distanceKmJemput = getEl('distance_km_jemput');
        if (distanceKmJemput) distanceKmJemput.addEventListener('input', updateTotalsJemput);
        ...
        if (proceedConfirmation) {
            proceedConfirmation.addEventListener('click', function() {
                ...
                if (deliveryMethod === 'jemput') {
                    ...
                    const distanceKm = getEl('distance_km');
                    const distanceKmJemput = getEl('distance_km_jemput');
                    if (distanceKmJemput && distanceKm) distanceKm.value = distanceKmJemput.value;
