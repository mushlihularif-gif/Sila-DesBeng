    ...
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
        <div class="card-header bg-white border-bottom py-3 px-4 d-flex justify-content-between align-items-center flex-wrap gap-2">
            ...
            <form method="GET" action="{{ route('lurah.laporan.index') }}" class="d-flex gap-2 align-items-center flex-wrap">
                ...
                <select name="filter_kecamatan_id" class="form-select form-select-sm rounded-pill border shadow-sm" style="width: auto; min-width: 150px;">
                    <option value="">Semua Kecamatan</option>
                    @foreach($kecamatanOptions as $opt)
                        <option value="{{ $opt->id }}" {{ request('filter_kecamatan_id') == $opt->id ? 'selected' : '' }}>{{ $opt->name }}</option>
                    @endforeach
                </select>
                <select name="filter_desa_id" class="form-select form-select-sm rounded-pill border shadow-sm" style="width: auto; min-width: 150px;">
                    <option value="">Semua Desa</option>
                    @foreach($desaOptions as $opt)
                        <option value="{{ $opt->id }}" {{ request('filter_desa_id') == $opt->id ? 'selected' : '' }}>{{ $opt->name }}</option>
                    @endforeach
                </select>
