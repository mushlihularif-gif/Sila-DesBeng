            ...
            <div class="col-xl-12">
                <div class="card modern-card shadow-sm">
                    ...
                    <div class="card-body p-4">
                        ...
                        <form id="formFasilitasUmum" action="{{ route('admin.unit.fasilitas_umum.update', $fasilitas->id) }}" method="POST" enctype="multipart/form-data">
                            @csrf
                            @method('PUT')
                                        ...
                                        <div class="input-group">
                                            <select class="form-select modern-input" id="kategori" name="kategori" required>
                                                ...
                                                @foreach($categories as $category)
                                                    <option value="{{ $category->name }}" {{ old('kategori', $fasilitas->kategori) == $category->name ? 'selected' : '' }}>
                                                        {{ $category->name }}
                                                    </option>
                                                @endforeach
                                            </select>
...
<!-- Section: Kapasitas & Satuan -->
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <div class="input-group">
                                            <select class="form-select modern-input" id="satuan" name="satuan" required>
                                                <option value="Unit" {{ old('satuan', $fasilitas->satuan ?? 'Unit') == 'Unit' ? 'selected' : '' }}>Unit</option>
                            ...
                            <div class="form-section mb-4">
                                ...
                                @php
                                    $assignedPengurusIds = old('pengurus_ids', $fasilitas->pengurus ? $fasilitas->pengurus->pluck('id')->toArray() : []);
                                @endphp
                                <div class="row g-3" id="pengurusCardsContainer">
                                    @forelse($pengurusList as $p)
                                        @php
                                            ...
                                            $isSelected = in_array($p->id, $assignedPengurusIds);
                                        @endphp
...
<script>
    ...
    document.getElementById('saveCategoryBtn')?.addEventListener('click', function() {
        ...
        if (newKategori) {
            ...
            fetch('{{ route("admin.categories.store") }}', {
                ...
                body: JSON.stringify({ name: newKategori, type: 'fasilitas' })
            })
                ...
                if (data.success && data.supir) {
                    ...
                    if (typeof Swal !== 'undefined') {
                        Swal.fire({
                            ...
                            text: 'Personil pengurus berhasil didaftarkan dan langsung ditugaskan. Silakan klik tombol "Simpan Data" di bagian bawah untuk menyimpan perubahan fasilitas ini.',
    ...
    // Inisialisasi hitungan pengurus yang terpilih saat halaman dibuka
    updateAssignedPengurusCount();
</script>
@endsection
