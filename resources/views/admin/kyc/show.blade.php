        ...
        <div class="col-xl-7 col-lg-7">
            <div class="card shadow mb-4">
                <div class="card-header py-3 d-flex flex-row align-items-center justify-content-between">
                    <h6 class="m-0 font-weight-bold text-primary">Hasil Ekstraksi OCR KTP</h6>
                    ...
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            <tr>
                                <th>NIK</th>
                                <td>{{ $kyc->user->nik ?? '-' }}</td>
                                <td class="font-weight-bold text-primary">{{ $kyc->nik_from_ocr ?? '-' }}</td>
                            </tr>
                            <tr>
                                <th>Nama Lengkap</th>
                                <td>{{ $kyc->user->name }}</td>
                                <td class="font-weight-bold text-primary">{{ $kyc->name_from_ocr ?? '-' }}</td>
                            </tr>
                            <tr>
                                <th>Jenis Kelamin</th>
                                <td>{{ $kyc->user->gender }}</td>
                                <td class="font-weight-bold text-primary">{{ ucfirst($kyc->gender_from_ocr) ?? '-' }}</td>
                            </tr>
                            <tr>
                                <th>Alamat</th>
                            ...
                            <tr>
                                ...
                                <td class="font-weight-bold text-primary">{{ $kyc->desa_from_ocr ?? '-' }}</td>
                            </tr>
                            <tr>
                                <th>Kecamatan</th>
                                <td>-</td>
                                <td class="font-weight-bold text-primary">{{ $kyc->kecamatan_from_ocr ?? '-' }}</td>
                            </tr>
                        </tbody>
                    </table>
                    ...
                    <div class="mt-4 border-top pt-4">
                        ...
                        <form action="{{ route('admin.kyc.approve', $kyc->id) }}" method="POST" class="d-inline" id="form-approve">
                            ...
                            <button type="button" class="btn btn-success" onclick="confirmApprove()">
                                <i class="fas fa-check"></i> Setujui & Update Profil
                            </button>
                        </form>
