            ...
            <div class="bg-white rounded-2xl shadow-lg border border-gray-200 overflow-hidden">
                ...
                <div class="px-8 py-6 {{ $valid ? 'bg-green-600' : 'bg-red-600' }} text-white text-center">
                    @if($valid)
                        ...
                        <p class="text-green-100 mt-1 text-sm">Surat ini diterbitkan secara resmi oleh Platform SiladesBeng</p>
                    @else
                        <svg class="mx-auto h-16 w-16 mb-3" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="px-8 py-6">
                    <dl class="space-y-4">
                        <div class="flex justify-between border-b border-gray-100 pb-3">
                            ...
                            <dd class="text-sm font-bold text-gray-900">#{{ str_pad($laporan->id, 5, '0', STR_PAD_LEFT) }}</dd>
                        </div>
                        <div class="flex justify-between border-b border-gray-100 pb-3">
                            <dt class="text-sm font-medium text-gray-500">Nama Pelapor</dt>
                            <dd class="text-sm text-gray-900">{{ $laporan->nama }}</dd>
                        </div>
                        <div class="flex justify-between border-b border-gray-100 pb-3">
                            <dt class="text-sm font-medium text-gray-500">Kategori</dt>
                            <dd class="text-sm text-gray-900">{{ $laporan->kategori }}</dd>
                        </div>
                        <div class="flex justify-between border-b border-gray-100 pb-3">
                            <dt class="text-sm font-medium text-gray-500">Tanggal Laporan</dt>
                            <dd class="text-sm text-gray-900">{{ $laporan->created_at->format('d F Y, H:i') }} WIB</dd>
                        </div>
                        <div class="flex justify-between border-b border-gray-100 pb-3">
                            <dt class="text-sm font-medium text-gray-500">Status Terkini</dt>
                            <dd class="text-sm font-bold text-gray-900">{{ $laporan->status }}</dd>
                        </div>
                        <div class="flex justify-between border-b border-gray-100 pb-3">
                            <dt class="text-sm font-medium text-gray-500">Tingkat Eskalasi</dt>
                            <dd class="text-sm font-bold text-gray-900 uppercase">{{ $laporan->escalation_level }}</dd>
                        </div>
                        <div class="flex justify-between bg-blue-50 p-3 rounded-md border border-blue-100">
                            <dt class="text-sm font-medium text-blue-800">Ditandatangani Oleh (Sistem)</dt>
                            <dd class="text-sm font-bold text-blue-900">{{ $handler_name }} (Admin {{ ucfirst($laporan->escalation_level) }})</dd>
                        </div>
                    </dl>
                </div>
                @endif
